...
impl BenchConfig {
    fn from_env() -> Result<Self, String> {
        let config = Self {
            ...
            max_neighbors: env_usize("OMEGA_BENCH_MAX_NEIGHBORS", 10)?,
            ef_construction: env_usize("OMEGA_BENCH_EF_CONSTRUCTION", 64)?,
            ef_search: env_usize("OMEGA_BENCH_EF_SEARCH", 56)?,
            candidate_window: env_usize("OMEGA_BENCH_RERANK", 24)?,
            query_memory_budget_mb: env_usize("OMEGA_BENCH_QUERY_MEMORY_MB", 4)?,
        };
...
fn temp_index() -> PathBuf {
    ...
    env::temp_dir().join(format!(
        "omega-vector-compact-bench-{}-{nonce}",
        std::process::id()
    ))
}
...
fn exact_top_k(query: &[f32], vector_count: u32, dimensions: usize, k: usize) -> Vec<u64> {
    ...
    let mut scored = Vec::with_capacity(vector_count as usize);
    for id in 0..vector_count as u64 {
        let candidate = normalize(&deterministic_vector(id, dimensions));
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let config = IndexConfig {
        ...
        rerank_candidates: bench.candidate_window,
        query_memory_budget_mb: bench.query_memory_budget_mb,
    };

    let mut index = OmegaCompactVectorIndex::open(&dir, config)?;
    let build_started = Instant::now();
    for id in 0..bench.vectors as u64 {
        index.add(id, &deterministic_vector(id, bench.dimensions))?;
    }
    index.flush()?;
    ...
    for query_index in 0..bench.queries {
        ...
        query_latencies.push(elapsed);
        total_search_duration += elapsed;
    ...
    let recall_at_k = recall_hits as f64 / recall_total as f64;
    let search_qps = bench.queries as f64 / total_search_duration.as_secs_f64();
    ...
    println!("  \"engine\": \"omega-vector-rs-compact\",");
    println!("  \"representation\": \"q8-only\",");
    ...
    println!("  \"candidate_window\": {},", bench.candidate_window);
    println!(
