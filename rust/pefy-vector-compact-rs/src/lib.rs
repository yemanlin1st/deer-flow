...
//! PEFY ΩVECTOR-RS Compact.
//!
//! Shadow q8-only vector engine for memory-constrained NANO/EDGE runtimes.
//! It deliberately keeps the validated BALANCED ΩVECTOR-RS format unchanged
//! while providing an A/B-compatible engine with no f16 corpus.
...
const META_MAGIC: &[u8; 8] = b"PEFYCMP1";
...
const Q8_SCALE_SQUARED: f32 = 16_129.0;
...
pub struct IndexConfig {
    ...
    pub ef_search: usize,
    /// Accepted for ABI/config compatibility with ΩVECTOR-RS BALANCED. In
    /// compact mode it controls the retained approximate candidate window.
...
pub struct OmegaCompactVectorIndex {
    dir: PathBuf,
    config: IndexConfig,
    count: u32,
    capacity: u32,
...
impl OmegaCompactVectorIndex {
    ...
    pub fn add(&mut self, external_id: u64, vector: &[f32]) -> Result<u32> {
        ...
        let node = self.count;
        let encoded = encode_q8(vector)?;
        self.write_q8(node, &encoded);
    ...
    pub fn search(&self, query: &[f32], k: usize) -> Result<Vec<SearchHit>> {
        ...
        let query_q8 = encode_q8(query)?;
        let requested_ef = self
            .config
            .ef_search
        ...
        for (score, node) in candidates {
            ...
            hits.push(SearchHit {
                ...
                score: score as f32 / Q8_SCALE_SQUARED,
                node,
            });
            if hits.len() >= k {
                break;
    ...
    pub fn stats(&self) -> IndexStats {
        ...
        let misc_per = 8 + 1;
        let per = q8_per + graph_per + misc_per;
        IndexStats {
            count: self.count,
    ...
    fn ensure_capacity(&mut self, required: u32) -> Result<()> {
        ...
        drop((old_q8, old_graph, old_ids, old_tombstones));
    ...
    fn write_q8(&mut self, node: u32, encoded: &[u8]) {
        let start = node as usize * self.config.dimensions;
        self.q8[start..start + self.config.dimensions].copy_from_slice(encoded);
    }
    ...
    fn dot_q8_query(&self, node: u32, query: &[u8]) -> i64 {
        dot_q8(self.q8_row(node), query)
    }

    fn dot_q8_nodes(&self, a: u32, b: u32) -> i64 {
        dot_q8(self.q8_row(a), self.q8_row(b))
    }
...
fn dot_q8(a: &[u8], b: &[u8]) -> i64 {
    a.iter()
        .zip(b)
        .map(|(&left, &right)| {
            let left = left as i16 - 128;
            let right = right as i16 - 128;
            left as i64 * right as i64
        })
        .sum()
}

fn encode_q8(vector: &[f32]) -> Result<Vec<u8>> {
    let sum: f64 = vector
        .iter()
        .map(|&value| {
...
fn map_rw(file: &File) -> std::io::Result<MmapMut> {
    // SAFETY: mappings are held by the index, and all resize operations drop
    // existing mappings before changing backing-file lengths.
